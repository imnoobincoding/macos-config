...
static inline mach_port_t mach_get_bs_port() {
    ...
    // Validate 'name' to contain only allowed characters
    for (size_t i = 0; i < strlen(name); ++i) {
        if (!isalnum(name[i]) && name[i] != '_') {
            fprintf(stderr, "Invalid characters in BAR_NAME environment variable.\n");
            return 0;
        }
    }
    ...
    if (bootstrap_look_up(bs_port, buffer, &port) != KERN_SUCCESS) {
        fprintf(stderr, "Failed to look up bootstrap service for '%s'.\n", buffer);
        free(buffer);
        return 0;
    }
...
static inline bool mach_send_message(mach_port_t port, const char* message, uint32_t len) {
    if (!message || !port) {
        fprintf(stderr, "Invalid message or port.\n");
        return false;
    }
    ...
    if (err != KERN_SUCCESS) {
        fprintf(stderr, "mach_msg failed with error: %d\n", err);
        return false;
    }
...
static inline uint32_t format_message(const char* message, char* formatted_message) {
    ...
    for (size_t i = 0; i < message_length; ++i) {
        ...
        caret++;
        if (caret >= FORMATTED_MESSAGE_SIZE - 1) {
            fprintf(stderr, "Formatted message exceeds buffer size.\n");
            break;
        }
    }

    // Ensure null termination
    if (caret < FORMATTED_MESSAGE_SIZE) {
        formatted_message[caret] = '\0';
    } else {
        formatted_message[FORMATTED_MESSAGE_SIZE - 1] = '\0';
    }
...
static inline void sketchybar(const char* message) {
    ...
    if (!g_mach_port) {
        ...
        if (!g_mach_port) {
            fprintf(stderr, "Failed to obtain Mach port.\n");
            return;
        }
    }

    if (!mach_send_message(g_mach_port, formatted_message, length)) {
        fprintf(stderr, "First attempt to send message failed. Retrying...\n");
        g_mach_port = mach_get_bs_port();
        if (!g_mach_port) {
            fprintf(stderr, "Failed to obtain Mach port on retry.\n");
            return;
        }
        if (!mach_send_message(g_mach_port, formatted_message, length)) {
            fprintf(stderr, "Second attempt to send message failed. Exiting.\n");
            exit(EXIT_FAILURE);
        }
    }
}
